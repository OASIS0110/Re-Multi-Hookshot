...
if (world.scoreboard.getObjective('floatTime') == undefined) world.scoreboard.addObjective('floatTime', 'float time');
if (world.scoreboard.getObjective('noFallingDamage') == undefined) world.scoreboard.addObjective('noFallingDamage', 'no falling damage');

system.run(function tick() {
	system.run(tick);
	// ロードしているディメンションに対して実行
	for (const dimension of loadingDimension()) {
		...
		for (const entity of world.getDimension(dimension).getEntities()) {
			// 落下ダメージ無効
			if (world.scoreboard.getObjective('noFallingDamage')?.hasParticipant(entity) === true) {
				const score = world.scoreboard.getObjective('noFallingDamage');
				if (score?.getScore(entity) ?? 0 >= 0) score?.addScore(entity, -1);
				if (score?.getScore(entity) ?? 0 < 0) score?.removeParticipant(entity);
			}
		}
	}
	...
	for (const player of world.getAllPlayers()) {
		...
		if (hasItem?.hasItem() && hasItem?.typeId == 're_hookshot:hookshot') {
			// 説明追加
			let lore = [
				'',
		...
		// Float
		if (hasItem?.hasItem() && hasItem?.typeId == 're_hookshot:hookshot' && player.isSneaking === true) {
			if ((0 <= floatTime)) {
...
/*
system.runInterval(() => {
	for (const entity of world.scoreboard.getObjective('noFallingDamage')?.getParticipants().keys.length) {
		//world.scoreboard.getObjective('noFallingDamage')?.getParticipants()[i].getEntity()?.addEffect('slow_falling', 1, {amplifier:0, showParticles: false})
	}
}, 3)
*/

system.runInterval(() => {
	//world.getAllPlayers()[0].sendMessage(`${JSON.stringify(world.scoreboard.getObjective('noFallingDamage')?.getParticipants())}`);
	//for (let i = 0;world.scoreboard.getObjective('noFallingDamage')?.getScores.length;i++)
},20)
